package com.monstrous.lightning3D.lightning;
...
public class LightningBolt {
    ...
    private static final float BOLT_WIDTH = 0.01f;
    ...
    private void makeBolt(Vector3 start, Vector3 end, float widthFactor) {
        ...
        points.add(new Vector3(end));
    }

    private void wiggleBolt() {
        Vector3 offsetA = new Vector3();
        Vector3 offsetB = new Vector3();
    ...
    private void buildModelInstance( Vector3 cameraPosition ) {
        // now build a model
        if(boltModel != null)
            boltModel.dispose();
        ...
        builder.setColor(Color.WHITE);  // change if you want coloured lightning
        for(int i = 0; i < points.size-1; i++){
            Vector3 from = points.get(i);
            Vector3 to = points.get(i+1);

            float width = BOLT_WIDTH*widthFactor*cameraPosition.dst(from);  // make rectangles wider if they are further away

            rect(builder, cameraPosition, from, to, width);
        }
    ...
    public void render( ModelBatch modelBatch, Vector3 cameraPosition){
        if(alpha > 0) {
            wiggleBolt();
            buildModelInstance(cameraPosition);
